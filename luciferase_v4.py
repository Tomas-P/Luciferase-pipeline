...
# this has to initiated in order for most things to work
...
def locate_plants(mask, plant_width, plant_height, fname=''):
    ...
    for x in range(0,mask.getWidth(),plant_width):
        for y in range(0,mask.getHeight(),plant_height):
            ...
            if poly and poly.statistics.area > 10:
                rm.addRoi(poly)
    if fname:
...
class LabeledScale(tk.Frame):
    ...
    def __init__(self, master, high, low, label, var):
        ...
        self.scale = tk.Scale(self, from_=low, to=high,orient=tk.HORIZONTAL, variable=var)
...
class UserInterface:
    ...
    def user_input(self, master):
        ...
        sequence_entry.grid(columnspan=3)
        mask_entry.grid(columnspan=3)

        custom_roi_entry.grid(columnspan=3)
        use_custom_box.grid()

        roi_entry.grid(columnspan=3)
        roi_check.grid(columnspan=3)

        group_entry.grid(columnspan=3)

        background_label.grid(columnspan=3)
        bx_scale.grid(columnspan=3)
        by_scale.grid(columnspan=3)
        b_width_scale.grid(columnspan=3)
        b_height_scale.grid(columnspan=3)
