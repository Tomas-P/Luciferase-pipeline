...
from Setaria import getinputs, sort_pixels
...
def create_selections():
	...
	for x in range(0,mask.getWidth(),width):
		for y in range(0,mask.getHeight(),height):
			...
			if r:
				mask.setRoi(r)
				if r.getStatistics().areaFraction < 60 or r.getStatistics().area < 0.125 * width * height:
					continue
				rm.addRoi(r)	
	return rm
...
def main():
	...
	archive = folder + "/RoiSet.zip"
	roi_m.runCommand("Save",archive)
	return roi_m

if sys.argv[0].endswith("At_seeds.py"):
	main()
